    ...
    .awaitAll(function(error, data) {
        ...
        function formatAllData(data) {
            ...
            data.forEach(function(arr) {
                var indicator = arr[0].indicator.split(" ")[0].replace(",", "").toLowerCase();  // get the indicator and format the key
                arr.forEach(function(obj) {
                    var region = obj.region;    // get current region
                    for (var year in obj) {     // parse through every year, add that region's data to that year array
                        if (parseInt(year)) {
            ...
            for (var year in yearObj) {     // remove years that don't have complete data sets for any region
